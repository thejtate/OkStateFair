...
<div id="node-<?php print $node->nid; ?>" class="content-wrapper <?php print $classes; ?> "<?php print $attributes; ?>>
    <div class="logo">
        <?php print theme('image', array('path' => path_to_theme() . '/images/logo-disney-2.jpg')); ?>
    </div>
    <div class="content-text">
    <div class="content-item">
      <h3>Sept. 14 - 19<?php //print $content['start']; ?></h3>
        ...
        <?php if (!empty($node->field_sf_event_location[LANGUAGE_NONE])): ?>
            <?php foreach ($node->field_sf_event_location[LANGUAGE_NONE] as $key => $val): ?>
                <?php if (!empty($val['taxonomy_term']->field_sf_event_location_map[LANGUAGE_NONE][0]['uri'])): ?>
                    <?php print l($val['taxonomy_term']->name, file_create_url($val['taxonomy_term']->field_sf_event_location_map[LANGUAGE_NONE][0]['uri']), array('external' => TRUE, 'attributes' => array('class' => array('link-where')))); ?>
                <?php else: ?>
                    <span class="link-where"><?php print $val['taxonomy_term']->name; ?></span>
                <?php endif; ?>
            <?php endforeach; ?>
        <?php endif; ?>
  ...
  <div class="content-text">
    ...
    <div class="content-item item-even">
<!--      <div class="share-wrapper social-type">-->
<!--        --><?php //if (!empty($node->field_sf_event_location[LANGUAGE_NONE])): ?>
<!--          --><?php //foreach ($node->field_sf_event_location[LANGUAGE_NONE] as $key => $val): ?>
<!--            --><?php //if (!empty($val['taxonomy_term']->field_sf_event_location_map[LANGUAGE_NONE][0]['uri'])): ?>
...
<!--        </ul>-->
<!--      </div>-->
      <?php print render($content['field_sf_event_media'][0]['field_disnep_subpage_mobile_desc']); ?>
    </div>
    <div class="content-item item-odd">
      <?php print render($content['field_sf_event_media'][0]['field_disnep_subpage_bottom_left']); ?>
    </div>
    <div class="content-item item-even">
      <?php print render($content['field_sf_event_media'][0]['field_tickets_text_mobile']); ?>
    </div>
