...
foreach ($rows as $id => $row):
  $year = date('Y', strtotime($row->field_blog_date_value_summary));
  $rows[$id]->field_blog_date_value_summary = $year;  
  $years[$id] = $year;
endforeach;
...
<ul class="dropdown">
  <?php foreach ($rows as $id => $row): ?>
    <?php if ($curr == $row->field_blog_date_value_summary && $year != TRUE) : ?>
      <?php
      print '<li>' . l($curr . '<span class="close"></span>', 'javascript:void();', array('external' => TRUE, 'html' => TRUE)) . '<ul>';
      $year = TRUE;
    elseif ($curr != $row->field_blog_date_value_summary) :
      $i = $i + 1;
      $curr = $years[$i];
