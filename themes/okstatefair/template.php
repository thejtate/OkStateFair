...
define('WINE_NID', 3877);
define('BANDSHEL', 2248);
define('FFF', 1548);
define('OHIIO', 2244);
define('SFP_MAP_NID', 3034);
define('SFP_TID', 653);
define('CHICKASAW_NID', 7176);
define('CHICKASAW_NID_LIVE', 11416);
define('OKSTATEFAIR_CREATIVE_ARTS_COMPETITIONS_NID', 2295);
define('OKSTATEFAIR_WIRELESS_INTERNET_POLICY_NID', 6292);
define('OKSTATEFAIR_SF_WIFI_NID', 17559);
define('OKSTATEFAIR_JFK_EXHIBITION_NID', 17782);
 ...
 */
function okstatefair_preprocess_page(&$vars, $hook) {
  drupal_add_js(array('okstatefair' => array('fair_open' => '2017/09/' . FAIR_START . ' 08:00')), array('type' => 'setting'));
  if (isset($vars['action_links'])) {
    foreach ($vars['action_links'] as $k => $v) {
      $v['#link']['title'] = 'Clone content';
      $vars['action_links'][$k]['#link']['title'] = 'Duplicate event';
    }
  }
  drupal_add_js('sites/all/libraries/jquery.fancybox-1.3.4/fancybox/jquery.fancybox-1.3.4.pack.js', 'file');
  drupal_add_css('sites/all/libraries/jquery.fancybox-1.3.4/fancybox/jquery.fancybox-1.3.4.css', 'file');
  if (isset($vars['node'])) {
    $node = $vars['node'];
    //$vars['site_type'][= ;
    switch ($node->type) {
      case 'homesubpark':
        $vars['site_type'] = 'page-type-3';
      ...
      case 'state_fair_event_combined':
        if ($node->nid == XTREME_BULLS || $node->nid == XTREME_CLAY ||  $node->nid == XTREME_JOSH || $node->nid == XTREME_JN || $node->nid == XTREME_MONTGOMERY_GENTRY || $node->nid == XTREME_ELI_YOUNG_BAND || $node->nid == XTREME_NEW) {
          $vars['pixel'] = TRUE;
        }
        ...
        if (in_array($node->nid, $vars['chickasaw_nodes']) || _okstate_special_performer($node)) {
          $vars['site_type'] = 'page-type-promo navigation-sidebar-static';
          $vars['promo_image'] = 'sfpng.png';
        }
        ...
        if (okstatefair_check_disnep_nid($node->nid)) {
          global $user;
          $vars['site_type'] = 'page-type-promo page-type-custom-1 page-type-custom-disney';
          $vars['back_link'] = '';
          if (user_access('access future nodes links')) {
            $items = okstatefair_disnep_nodes();
            foreach ($items as $key => $value) {
              $items_links[] = l($key, 'node/' . $value);
            }
            $vars['year_links'] = theme('item_list', array('items' => $items_links, 'title' => '', 'type' => 'ul', 'attributes' => array('class' => array('year-modes'))));
          }
        }
        if (($node->nid == XTREME_BULLS) || ($node->nid == XTREME_JN) || ($node->nid == XTREME_JOSH)) {
      ...
      case 'state_fair_park_event':
        $dest = drupal_get_destination();
        if (!empty($dest['destination']) && ($dest['destination'] != 'node/' . $node->nid)) {
          $vars['back_link'] = '<a href="' . base_path() . $dest['destination'] . '" class="btn-back"><i class="icon-btn-back"></i>' . t('BACK') . '</a>';
        }
        else {
          $vars['back_link'] = l('<i class="icon-btn-back"></i>' . t('BACK'), OKSTATEFAIR_VIEWS_CALENDAR_STATE_FAIR_PARK_CALENDAR_PAGE_PATH, array('html' => TRUE, 'attributes' => array('class' => array('btn-back'))));
        }
        $vars['site_type'] = 'page-type-5';
        if (array_key_exists('okstatefair_custom_header_sf', $vars['page']['header'])) {
          $vars['page']['header']['okstatefair_custom_header_sf']['#access'] = FALSE;
        }
        if (array_key_exists('okstatefair_custom_footer_sf', $vars['page']['footer'])) {
      ...
      case 'content_links':
      case 'food_beverage':
        $state_fair_map = array(
          OKSTATEFAIR_SF_WIFI_NID,
        );

        if (!in_array($node->nid, $state_fair_map)) {
          $vars['site_type'] = 'page-type-6 page-type-v';
          ...
          $vars['site_type'] = 'page-type-4 page-type-v';
        }

        if ($node->type == 'barns') {
          $vars['site_type'] .= ' navigation-sidebar-static';
        }
        if ($node->type == 'wifi_ad') {

          try {
            $node = node_load(STATE_FAIR_PARK_NID);
            $node_wrapper = entity_metadata_wrapper('node', $node);
            $slides = array();
            foreach ($node_wrapper->field_homesub_slides->value() as $key => $value) {
              $slides['items'][] = l(theme('image', array('path' => $value['uri'])), $value['alt'] ? $value['alt'] : '', array('html' => TRUE));
            }
      ...
      if ($node->type == 'employment') {
        $vars['site_type'] .= ' page-type-v';
      }
    ...
    if ($vars['node']->nid == CHICKASAW_NID || $vars['node']->nid == CHICKASAW_NID_LIVE) {
      $vars['site_type'] = 'page-type-promo navigation-sidebar-static';
      $vars['promo_image'] = 'sfpng.png';
    }
    $check_back = (!empty($node->field_sf_event_check_back) &&
      !empty($node->field_sf_event_check_back[LANGUAGE_NONE][0]['value'])) ?
      $node->field_sf_event_check_back[LANGUAGE_NONE][0]['value'] : '';
    if ($check_back) {
      if (isset($vars['site_type'])) {
        $vars['site_type'] .= $vars['site_type'] . ' page-type-check-back';
      }
      else {
        $vars['site_type'] = 'page-type-check-back';
      }
    }
  }
  else {
    ...
    if (in_array('page__calendar_sfp', $vars['theme_hook_suggestions'])) {
      $vars['site_type'] = 'page-type-6';
      $vars['page']['footer']['okstatefair_custom_footer_sf']['#access'] = FALSE;
      $vars['page']['header']['okstatefair_custom_header_sf']['#access'] = FALSE;
    ...
    if (in_array('page__blogpost', $vars['theme_hook_suggestions'])) {
      $vars['site_type'] = 'page page-type-blog';
      $vars['theme_hook_suggestions'][] = 'page__press';
      $vars['page']['footer']['okstatefair_custom_footer_sf']['#access'] = FALSE;
      $vars['page']['header']['okstatefair_custom_header_sf']['#access'] = FALSE;
...
//      $vars['page']['footer']['okstatefair_custom_footer_2logo']['#markup'] = $block['content'];
  ...
  if ($view) {
    ...
    switch ($view_name) {
      case 'vendors':
        if (!user_access('okstate custom view vendors finder')) {
          $vars['site_type'] = !empty($vars['site_type']) ?
            $vars['site_type'] . ' page-type-check-back-views' : 'page-type-4 page-type-check-back-views';
        }
        break;
      case 'food_finder':
        if (!user_access('okstate custom view food finder')) {
          $vars['site_type'] = !empty($vars['site_type']) ?
            $vars['site_type'] . ' page-type-check-back-views' : 'page-type-4 page-type-check-back-views';
        }
        break;
  ...
  //Google Tag Manager
  $vars['google_tag_manager'] = '<!-- Google Tag Manager -->
    <noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-PRMXPM"
    height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
    <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({\'gtm.start\':
    new Date().getTime(),event:\'gtm.js\'});var f=d.getElementsByTagName(s)[0],
    j=d.createElement(s),dl=l!=\'dataLayer\'?\'&l=\'+l:\'\';j.async=true;j.src=
    \'//www.googletagmanager.com/gtm.js?id=\'+i+dl;f.parentNode.insertBefore(j,f);
    })(window,document,\'script\',\'dataLayer\',\'GTM-PRMXPM\');</script>
 ...
 */
function okstatefair_preprocess_node(&$vars) {
  $chickasaw_nodes = chickasaw_nodes();
  $node = $vars['node'];
  if ($node->status == 0) drupal_set_message('This page is currently not published. Please be sure to publish and add to the site menu for it to show publicly');
  $webform_node = node_load(WEBFORM_GOLDEN_TICKET_NID);
  $golden_ticket_nid = WEBFORM_GOLDEN_TICKET_NID;
  $includes_webform_path = drupal_get_path('module', 'webform') . '/includes/';
  require_once $includes_webform_path . 'webform.submissions.inc';
  $cl = 'text golden-ticket ctools-modal-modal-popup-large';
  if (_webform_submission_total_limit_check($webform_node)) {
    $golden_ticket_nid = WEBFORM_GOLDEN_TICKET_NEWSLETTER_NID;
    $cl .= ' newsletter';
  }
  $vars['golden_ticket'] = '<div class="golden-ticket-img" style="top: 469px; right: -42px;">' . ctools_modal_text_button(theme('image', array('path' => path_to_theme() . '/images/Flag.png')), 'modal_forms/nojs/webform/' . $golden_ticket_nid, 'ctools-modal-modal-popup-large', $cl) . '<div class="show-off">' . theme('image', array('path' => path_to_theme() . '/images/sparkl.png')) . '</div></div>';
  ...
  if (okstatefair_check_disnep_nid($node->nid)) {
    ...
    if (isset($_GET['draft'])) {
      if (isset($_GET['vid'])) {
        $current_node = node_load(1973,$_GET['vid']);
      } else {
        $current_node = workbench_moderation_node_current_load(node_load(1973));
      }
      $current_node2 = node_view($current_node);
      foreach ($current_node2 as $key => $value) {
        if (preg_match('/field_/', $key) || $key == 'links') {
          $vars['content']['field_sf_event_media'][0][$key] = $current_node2[$key];
        }
      }
    }
  }
  ...
  switch ($node->type) {
    case 'home':
      ...
      break;
    case 'instagram_winner':
      if  (isset($node->field_instagram_winner_date[LANGUAGE_NONE]['0']['taxonomy_term'])) {
        if (!empty($node->field_instagram_winner_link)) {
          $vars['content']['field_instagram_winner_link']['0']['#element']['attributes']['target'] = '_blank';
          $vars['content']['field_instagram_winner_image']['0']['#path']['options']['attributes']['alt'] = render($vars['content']['field_instagram_winner_link']);
        }
      $date= $node->field_instagram_winner_date[LANGUAGE_NONE]['0']['taxonomy_term'];
      $date_field = $date->field_sf_term_date[LANGUAGE_NONE]['0'];
      $date_obj = new DateObject($date_field['value'], $date_field['timezone_db'], date_type_format($date_field['date_type']));
      if (!empty($node->field_instagram_winner_image)) {
        $vars['classes_array'][] = 'with-photo';
        $vars['imgpath'] = $vars['field_instagram_winner_image']['0']['uri'];
        $vars['day_title'] = l($date_obj->format('F j'), file_create_url($vars['imgpath']), array('attributes' => array('alt' => render($vars['content']['field_instagram_winner_link']))));
      }
      else {
        $vars['day_title'] = $date_obj->format('F j');
        $vars['imgpath'] = '';
      }
      if($vars['view_mode'] == 'insta_home') {
        $vars['theme_hook_suggestions'][] = 'node__' . $vars['type'] . '__insta_home';
        $vars['day_title'] = $date_obj->format('F j');
      }
      }
      break;
    case 'state_fair_event':
      ...
      $vars['mail'] = l(t('mail'), 'mailto:?subject=' . $node->title . '&body=' . $url_for_share . ' ' . $text, array('external' => TRUE));
      if (okstatefair_check_disnep_nid($node->nid) || $node->nid == XTREME_BULLS) {
        $vars['back_link'] = '';
        $start = t('September') . ' ';
        $start = okstatefair_calendar_prepare_period($vars['node']->nid, $start);
        $vars['content']['start'] = $start;
      }
      elseif (!array_key_exists('start', $vars['content'])) {
        $vars['content']['start'] = '';
      }
      if (in_array($node->nid, $chickasaw_nodes) || in_array($node->nid, private_events()) || okstatefair_check_disnep_nid($node->nid) || $node->nid == XTREME_BULLS || _okstate_special_performer($node)) {
      ...
      switch ($node->nid) {
        ...
        case XTREME_JN:
        case XTREME_JOSH:
        case XTREME_MONTGOMERY_GENTRY:
        case XTREME_ELI_YOUNG_BAND:
          $vars['theme_hook_suggestions'][] = 'node__' . $vars['type'] . '__chickasaw__xtreme';
          break;
      }

      $referencing_entity_type = !empty($vars['referencing_entity']) ?
        $vars['referencing_entity']->type : '';

      if ($referencing_entity_type == 'state_fair_event_combined') {
        $vars['combined'] = TRUE;
        $first_section_nid = !empty($vars['referencing_entity']->field_sf_event_comb_items[LANGUAGE_NONE][0]) ?
          $vars['referencing_entity']->field_sf_event_comb_items[LANGUAGE_NONE][0]['nid'] : '';
        $vars['first_section'] = ($first_section_nid == $node->nid) ? TRUE : FALSE;

        if (!$vars['first_section']) {
          $vars['content']['field_sf_event_dates']['#access'] = FALSE;
        }
        else {
          $url_for_share = urlencode(url('node/' . $vars['referencing_entity']->nid, array('absolute' => TRUE)));
          $combine_title = urlencode($vars['referencing_entity']->title);

          $vars['twitter'] = l(t('Twitter'), 'http://twitter.com/share?url=' . $url_for_share . '&text=' . $combine_title, array('external' => TRUE));
          $vars['fb'] = l(t('FB'), 'https://www.facebook.com/sharer.php?u=' . $url_for_share . '&t=' . $combine_title, array('external' => TRUE));
          $vars['mail'] = l(t('mail'), 'mailto:?subject=' . $combine_title . '&body=' . $url_for_share, array('external' => TRUE));

          $vars['content']['field_sf_event_dates'] = field_view_field('node', $vars['referencing_entity'], 'field_sf_event_dates');
          $vars['combined_title'] = $vars['referencing_entity']->title;
        }
      }
      break;
    case 'barnyard':
    ...
    case 'wifi_ad':
      try {
        $node = ($vars['node']->nid == OKSTATEFAIR_SF_WIFI_NID) ? node_load(STATE_FAIR_NID): node_load(STATE_FAIR_PARK_NID);
        $node_wrapper = entity_metadata_wrapper('node', $node);
        $slides = array();
        foreach ($node_wrapper->field_homesub_slides->value() as $key => $value) {
          $img = theme('image', array('path' => $value['uri']));
        ...
        if (isset($slides['items']) && !empty($slides['items'])) {
          $slider = theme('item_list', $slides);
          $pager = theme('item_list', $pages);
          $vars['homesub_slides'] = $slider;
          $vars['homesub_pages'] = $pager;
        }
      }
      catch (EntityMetadataWrapperException $exc) {
        watchdog(
...
//      }
      ...
      $current_terms = isset($_GET['field_vendors_category_tid']) ? $_GET['field_vendors_category_tid'] : '';
      $step_terms = array();
        ...
        foreach ($value as $terms_value) {
          if (!array_key_exists($terms_value, $description)) {
            if ($current_terms == $terms_value) {
              ...
              if (count(taxonomy_term_load($terms_value)->name) > 0) {
...
//        $vars['description'] = str_replace($description[$current_terms], '<span class="selected">' . $description[$current_terms] . '</span>', implode(", ", $description));
...
//          }
    ...
    case 'state_fair_park_event':
      ...
      if (!empty($node->field_sfp_event_dates[LANGUAGE_NONE])) {
        ...
        if (!empty($entity) && is_array($entity)) {
          foreach ($entity as $key => $val) {
            ...
            $timezone_default = 'America/Monterrey';
            if (!empty($value_1)) {
      ...
      $calendar_arg = NULL;
      if(!empty($first_date)){
        $calendar_arg = $first_date->format('Y-m');
      }
      $vars['calendar'] = views_embed_view('state_fair_park_calendar', $display_id = 'page_1', $calendar_arg);
      break;
    case 'employment':
      if ($node->nid == OKSTATEFAIR_WIRELESS_INTERNET_POLICY_NID) {
        $vars['show_jobs'] = FALSE;
      }
      break;
    case 'map':
      $vars['check_back'] = (!empty($vars['content']['field_sf_event_check_back']) &&
        !empty($vars['content']['field_sf_event_check_back']['#items'][0]['value'])) ?
        $vars['content']['field_sf_event_check_back']['#items'][0]['value'] : '';
 ...
 * @see field.tpl.php
 */
function okstatefair_preprocess_field(&$vars, $hook) {
  switch ($vars['element']['#field_name']) {
    case 'field_media_audio_list' :
      if(!empty($vars['element']['#object']->field_media_audio_artist[LANGUAGE_NONE][0]['value'])) {
        //replace song title in link, from audio_artist field if exist
        if(!empty($vars['items'][0]['#tracks'][0]) && !empty($vars['element']['#object']->field_media_audio_list[LANGUAGE_NONE][0]['file']->filename)) {
          $vars['items'][0]['#tracks'][0] = str_replace($vars['element']['#object']->field_media_audio_list[LANGUAGE_NONE][0]['file']->filename . '</a>', $vars['element']['#object']->field_media_audio_artist[LANGUAGE_NONE][0]['value'] . '</a>', $vars['items'][0]['#tracks'][0]);
        }
      }
      break;
...
//      $views_date_arg_name = okstatefair_custom_get_default_date(CALENDAR_SHARED_EVENTS_DATE_FORMAT);
//      $views_date_arg_name = '09/11/2014';
//      $views_date_arg = taxonomy_get_term_by_name($views_date_arg_name, 'state_fair_event_dates');
//      $views_date_arg = element_children($views_date_arg);

//      $vars['instagram'] = views_embed_view('winners', 'block_1', $views_date_arg['0']);
      break;
    ...
    case 'field_sponsorship_proud_sponsors' :
      $field_obj = $vars['items'];
      foreach ($field_obj as $key => $value) {
        $vars['items'][$key]['links']['#access'] = FALSE;
      }
    ...
    case 'field_homesub_foodfinder' :
      $vars['foodfinder'] = variable_get('foodfinder', '');
      break;
      case 'field_discount_pdf' :
          $field_obj = $vars['items'];
          foreach ($field_obj as $key => $value) {
              $vars['items'][$key]['description'] = $vars['element']['#items'][$key]['description'];
             // $vars[']['#items'][$key]['desc']['#access'] = FALSE;
          }
      break;
    case 'field_artex_gallery':
      if (isset($vars['items']) && !empty($vars['items'])) {
        foreach ($vars['items'] as &$image) {
          if (isset($image['#path'])) {
            $title = isset($image['#item']['alt']) ? $image['#item']['alt'] : '';
            $id = isset($vars['element']['#object']->item_id) ? $vars['element']['#object']->item_id : '';
            $image['#path']['options'] = array(
              'attributes' => array(
                'class' => 'construction_updates_gallery-popup',
                'title' => $title,
                'rel' => 'construction_updates_gallery-' . $id,
                )
            );
          }
        }
      ...
      break;
    case 'field_com_cont_items':
      $vars['classes_array'][] = (isset($vars['items']) && (count($vars['items']) > 1)) ? 'content-items-inner' : '' ;
      break;
    case 'field_com_cont':
      $nid = isset($vars['element']['#object']->nid) ? $vars['element']['#object']->nid : '';
      if (($nid == OKSTATEFAIR_CREATIVE_ARTS_COMPETITIONS_NID) && isset($vars['items'][0]['#attributes']['class'])) {
        $vars['items'][0]['#attributes']['class'][] = 'width-600-right';
      }
      break;
    case 'field_bandday_content_image':
      if (isset($vars['items']) && (count($vars['items']) == 1)) {
        $uri = isset($vars['items'][0]['#item']['uri']) ? $vars['items'][0]['#item']['uri'] : '';
        $alt = isset($vars['items'][0]['#item']['alt']) ? $vars['items'][0]['#item']['alt'] : '';
 ...
 * theme_item_list()
 */
function okstatefair_item_list($variables) {
  $attributes = array();
 ...
 */
function okstatefair_preprocess_views_view(&$vars) {

  switch($vars['view']->name) {
    case 'food_finder':
      $vars['view']->food_custom_block = okstatefair_food_custom_block();
      ...
      $vars['check_back_image'] = theme('image', $check_back_image);
      break;
    case 'state_fair_park_calendar':
      if ($vars['view']->exposed_raw_input['sfp_category'] != 'All') {
      ...
      $vars['view']->next_year = l(t('Next'), OKSTATEFAIR_VIEWS_CALENDAR_STATE_FAIR_PARK_CALENDAR_PAGE_PATH . '/' . ($date[0] + 1) . '-' . $date[1], array('absolute' => TRUE, 'query' => $query_args, 'attributes' => array('rel' => 'nofollow', 'class' => array('btn-next'))));
      $vars['view']->prev_year = l(t('Prev'), OKSTATEFAIR_VIEWS_CALENDAR_STATE_FAIR_PARK_CALENDAR_PAGE_PATH . '/' . ($date[0] - 1) . '-' . $date[1], array('absolute' => TRUE, 'query' => $query_args, 'attributes' => array('rel' => 'nofollow', 'class' => array('btn-prev'))));
      $vars['view']->display_year = $date[0];
      $vars['view']->display_month = date("F", mktime(0, 0, 0, $date[1], 10));
      if ($vars['view']->use_ajax) {
        drupal_add_js(path_to_theme() . '/js/ajax_view.js', 'file');
      }
      break;
    case 'calendar_chosen_month':
    ...
    case 'calendar_chosen_date':
      ...
      break;
    case 'vendors':
      $vars['no_vendors_access'] = !user_access('okstate custom view vendors finder');
      $path = drupal_get_path('theme', 'okstatefair') . '/images/coming_soon.jpg';
      $check_back_image = array(
        'path' => $path,
        'alt' => t('Comming Soon'),
...
function okstatefair_preprocess_views_view_table(&$vars) {
  switch ($vars['view']->name) {
    case 'sfp_calendar_list':
      //  _okstatefair_union_nearest_events($vars, 'field_sfp_event_dates_date');
      okstatefair_calendar_group_nearest_events($vars, 'field_sfp_event_dates_date', TRUE, TRUE);
      break;
      ...
      if (!empty($vars['rows'])) {
        foreach ($vars['rows'] as $key => $val) {
          ...
          $date_obj = new DateObject(
            $vars['view']->result[$key]->field_field_sfp_event_dates_date['0']['raw']['value'],
            $vars['view']->result[$key]->field_field_sfp_event_dates_date['0']['raw']['timezone_db'],
            date_type_format($vars['view']->result[$key]->field_field_sfp_event_dates_date['0']['raw']['date_type']));
          $default_timezone = new DateTimeZone(date_default_timezone());
          $date_obj->setTimezone($default_timezone);
          $month = custom_month($date_obj->format('M')) . ' ' . $date_obj->format('d');
          $vars['rows'][$key]['field_sfp_event_dates_date'] = $month;
        }
      }
      break;
  }
...
function okstatefair_views_pre_render(&$view) {
  $view_name = !empty($view->name) ? $view->name : '';
  if ($view_name == 'state_fair_calendar') {
    $results = $view->result;
    $new_results = array();

    foreach($results as $key => $result) {
      $time = !empty($result->field_collection_item_field_data_field_sf_event_dates__field) ?
        $result->field_collection_item_field_data_field_sf_event_dates__field : '';
      $location = !empty($result->taxonomy_term_data_field_data_field_sf_event_location_tid) ?
        $result->taxonomy_term_data_field_data_field_sf_event_location_tid : '';
      $nid = !empty($result->nid) ? $result->nid : '';

      if (!empty($time)) {
        $time = strtotime($time);
        $new_key = $time . '_' . $location;

        if (!array_key_exists($new_key, $new_results)) {
          $new_results[$new_key] = $result;
        }
        else {
          $query = db_select('node', 'n')
            ->fields('n', array('nid', 'title'))
            ->condition('type', 'state_fair_event_combined')
            ->condition('status', 1);
          $query->join('field_data_field_sf_event_comb_items', 'event', 'n.nid = event.entity_id');
          $query->condition('event.field_sf_event_comb_items_nid', $nid);

          $query_result = $query->execute()
            ->fetchAssoc();

          if (!empty($query_result)) {
            $result->nid = $query_result['nid'];
            $result->node_title = $query_result['title'];
            $new_results[$new_key] = $result;
          }
          else {
            $new_results[] = $result;
...
function okstatefair_preprocess_search_result(&$variables) {
  global $base_url;
  if (isset($variables['result']['blog_post'])) {
    $variables['url'] = $variables['result']['blog_post_url'];
  }

  if (isset($variables['result']['custom_url']) && !empty($variables['result']['custom_url'])) {
    $variables['url'] = $variables['result']['custom_url'];
  }
 ...
 */
function okstatefair_food_custom_block() {
    ...
    if (!empty($term->field_food_categories)) {
      foreach ($term->field_food_categories[LANGUAGE_NONE] as $value_food) {
        if ($food_categories_tids['BACON'] == $value_food['tid']) {
          $bacon .= '  <div class="item">
    ' . l($term->name, 'food-finder', array('query' => array('field_vendors_category_tid' => $term->tid, 'bacon' => '1'))) . '
  </div>
';
        }
        if ($food_categories_tids['Deep Fried'] == $value_food['tid']) {
          $deep_fried .= '  <div class="item">
    ' . l($term->name, 'food-finder', array('query' => array('field_vendors_category_tid' => $term->tid, 'deep_fried' => '1'))) . '
  </div>
';
        }
        if ($food_categories_tids['On A Stick'] == $value_food['tid']) {
          $on_a_stick .= '  <div class="item">
    ' . l($term->name, 'food-finder', array('query' => array('field_vendors_category_tid' => $term->tid, 'on_a_stick' => '1'))) . '
  </div>
';
        }
        if ($food_categories_tids['GTOAF'] == $value_food['tid']) {
          $gtof .= '  <div class="item">
    ' . l($term->name, 'food-finder', array('query' => array('field_vendors_category_tid' => $term->tid, 'gtoaf' => '1'))) . '
  </div>
';
        }
      }
  ...
  $bacon_class = (isset($_GET['bacon'])) ? ' active' : '';
  $on_a_stick_class = (isset($_GET['on_a_stick'])) ? ' active' : '';
  $deep_fried_class = (isset($_GET['deep_fried'])) ? ' active' : '';
  $gtof_class = (isset($_GET['gtoaf'])) ? ' active' : '';
  $bacon = '<div class="filter filter-a' . $bacon_class . '">
  <label>' . t('BACON') . '</label>
  <div class="widget">
...
';
  $on_a_stick = '<div class="filter filter-b' . $on_a_stick_class . '">
  <label>' . t('On-a-Stick') . '</label>
  <div class="widget">
    <div class="form-item form-type-select">
' . $on_a_stick . '</div>
</div></div>
';
  $deep_fried = '<div class="filter filter-c' . $deep_fried_class . '">
  <label>' . t('DEEP-FRIED') . '</label>
  <div class="widget">
    <div class="form-item form-type-select">
' . $deep_fried . '</div>
</div></div>
';
  ...
  $gtof = '<div class="filter filter-d' . $gtof_class . '">
      <label>' . t('Great Taste of a Fair') . '</label>
    <div class="widget">
    <div class="form-item form-type-select">
' . $gtof . '</div>
</div></div>';
 ...
 * @return HTML string representing the form element.
 */
function okstatefair_select_as_checkboxes($vars) {
  $element = $vars['element'];
  ...
  $output = '<div class="bef-checkboxes form-sort">';
  foreach ($element['#options'] as $option => $elem) {
    if ('All' === $option) {
 ...
 */
function okstatefair_preprocess_views_view_field($vars) {
  //fix calendar date format
  if (!empty($vars['view']->name) && $vars['view']->name == 'state_fair_calendar') {
    if ($vars['field']->field == 'field_sf_event_dates_date') {

      if (!empty($vars['field']->options['id']) && $vars['field']->options['id'] == 'field_sf_event_dates_date_2') {

        $date_field = field_info_field('field_sf_event_dates_date');
        $date_obj = new DateObject(
                $vars['row']->field_field_sf_event_dates_date_2[0]['raw']['value'],
                $date_field['settings']['timezone_db'],
                date_type_format($date_field['type']));
        $default_timezone = new DateTimeZone(date_default_timezone());
        $date_obj->setTimezone($default_timezone);

        $formated_date = okstatefair_agenda_get_formated_date($date_obj);
        $vars['output'] = $formated_date;
        $vars['field']->last_render = $formated_date;
        $vars['field']->last_render_text = $formated_date;
      }
    }
  }
}